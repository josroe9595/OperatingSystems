...
class cpu_process:
	def __init__(self,proc_num,burst_time,num_bursts,io_time):
...
def run_processes(proc_lis):
	queue = []
	for proc in proc_lis:
		queue.append(proc)
	n = len(queue)
	t_cs = 0
	print "time", str(t_cs) + "ms: Simulator started [Q"+print_queue(queue)
	run_slot = []
	io_set = []
...
if __name__ == "__main__":
	...
	for line in words:
		final = line.split('|')
		proc_lis.append(cpu_process(final[0],final[1],final[2],final[3]))
	run_processes(proc_lis)
